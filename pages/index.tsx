...
// Icon Components inside file for simplicity
const SparklesIcon = () => (
...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    if (vendors.length > 0) { // Avoid overwriting with empty array on initial hydration if not careful
       localStorage.setItem('event_vendors', JSON.stringify(vendors));
    } else if (localStorage.getItem('event_vendors')) {
      // Logic handled in mount, but for updates:
      localStorage.setItem('event_vendors', JSON.stringify(vendors));
    }
  }, [vendors]);
